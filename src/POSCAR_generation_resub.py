...
def scale(compound):
    ...
    if len(seperate)==1:
        ...
        scale_factor = sum(element.atomic_radius[chars] for chars in seperate)*1.0/structure_and_lattice.compare_radius/2.0
        return math.sqrt(scale_factor)
#scale('P')
...
#
# resubmission, no need to create folders
# 
...
for file_name in files:
    fisrt_lev_path = os.path.join(folder_dir,file_name)
    files2 = os.listdir(first_lev_path)
    for file_lev_two in files2:
        file_name_list = file_lev_two.split('_')
        second_lev_path = os.path.join(first_lev_path,file_lev_two)
        if 'OSZICAR' not in os.listdir(second_lev_path):
            shutil.copy2(os.path.join(file_dir,'_'.join(POSCAR,file_lev_two)),os.path.join(second_lev_path,'POSCAR'))
        file_osz = open('OSZICAR','r')
        file_cont = file_osz.readlines()
        if len(file_cont) < 5:
            shutil.copy2(os.path.join(file_dir,'_'.join(POSCAR,file_lev_two)),os.path.join(second_lev_path,'POSCAR'))
        file_osz.close()