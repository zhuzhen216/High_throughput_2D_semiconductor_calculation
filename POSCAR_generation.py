...
class element:
    ...
    GroupVI = ['O','S','Se','Te']
    GroupVII = ['F','Cl','Br','I']
    atomic_mass={'B':10.811,'Al':26.5815,'Ga':69.723,'In':114.818,'C':12.0107,'Si':28.0855,'Ge':72.61,'Sn':118.710,'Pb':207.2,'N':14.0067,'P':30.9738,'As':74.9216,'Sb':121.76,'Bi':208.58,'O':15.9994,'S':32.066,'Se':78.96,'Te':127.60,'F':18.9984,'Cl':35.4527,'Br':79.504,'I':126.90447}
    atomic_radius = {'B':87,'Al':118,'Ga':138,'In':156,'C':67,'Si':111,'Ge':125,'Sn':145,'Pb':154,'N':56,'P':98,'As':114,'Sb':133,'Bi':143,'O':48,'S':88,'Se':103,'Te':123,'F':42,'Cl':79,'Br':94,'I':115}
...
class structure_and_lattice:
    ...
    frac=[[(0.75,0.5997,0.2229),(0.25,0.0997,0.0718),(0.25,0.014384,0.2094),(0.75,0.5144,0.0854)],[(0.0,0.0,0.0113),(0.333,0.333,-0.05132)]]
    lattice = [[(3.348,0.,0.),(0.,4.623,0.),(0.,0.,20.)],[(2.881076913,-1.662861501,0.0),(2.881077039,1.662861720,0.0),(0.0,0.0,20.)]]
    lattice_sites = [4,2]
    phase = [0,1]
...
# os.path module
# the idea is:
# - you will initialize a new path: new_folder=os.path.join(current_path,folder_name)
...
for folder_name in compound_all:
    for phase_name in compound_phase[folder_name]:
        ...
        create_folder(current_path,combined_name)   
...
#
file_dir = '/Users/zhenzhu/Project/calculcation/NaMnO2/HSE/transition_level/Al_Na/POSCAR'
